    ...
    <div class="col-12">
        <div class="card">
            <div class="card-header">
                ...
                <a href="{{ route('customer.create') }}" class="btn btn-outline-primary btn-sm ml-5">Tambah Customer</a>
            </div>
            @if(session()->has('msg'))
                    ...
                    @foreach ($customer as $index => $item)
                        <tr>
                            ...
                            <td class="text-center">
                                <a class="icon" href="{{ route('customer.edit', $item->id) }}" title="edit item">
                                    <i class="fe fe-edit"></i>
                                </a>
                                <a class="icon btn-delete" href="javascript:void(0)" data-id="{{ $item->id }}" title="delete item">
                                    <i class="fe fe-trash"></i>
                                </a>
                                <form action="{{ route('customer.destroy', $item->id) }}" method="POST" id="form-{{ $item->id }}">
                                    @csrf
                                    @method('delete')
                                </form>
                            </td>
